...
def insert_message(user_name, text, message_type, is_temp=False, audio_payload=None):
    ...
    # Write to Firestore
    try:
        # Optimization: We skip the optimistic broadcast for standard chat messages 
        # to prevent \"double updates\" and WebSocket thrashing on mobile. 
        # The Firestore listener (which is high-speed) will handle the UI update.
    ...
    try:
        ...
        for doc in docs:
            ...
            if (now_obj - past_time.replace(tzinfo=None)).total_seconds() > 300:
                deja_vu_ref = f\"\u2173 Connection identified: Originally stated by {past_user} on {past_time.strftime('%Y-%m-%d')}\"
                time.sleep(0.5)
                broadcast({
                    \"user_name\": \"ARCHIVE\",
                    \"text\": deja_vu_ref,
